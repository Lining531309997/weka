	...
	public final double newEnt(Distribution bags, double totalNoInst) {
		...
		for (i = 0; i < bags.numBags(); i++) {
			...
			if (bags.perBag(i) == 0) {
				...
				for (j = 0; j < bags.numClasses(); j++) {
					// TODO 陈英
					sum += bags.perClassPerBag(i, j) * (bags.perBag(i) - bags.perClassPerBag(i, j)) * (bags.perBag(i) + bags.total() * 0.1);
//					sum += bags.perClassPerBag(i, j) * (bags.perBag(i) - bags.perClassPerBag(i, j)) * (bags.perBag(i) + bags.total() * 0);
				}
...
//			for (j = 0; j < bags.numClasses(); j++) {
//				sum += bags.perClass(j);
//				product *= bags.perClass(j);
//			}
//			returnValue += DecimalCalculate.div(product, sum);
//		}
