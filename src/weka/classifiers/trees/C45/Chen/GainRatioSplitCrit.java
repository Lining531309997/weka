...
package weka.classifiers.trees.C45.Chen;
...
public final class GainRatioSplitCrit extends EntropyBasedSplitCrit {
	...
	public final double splitCritValue(Distribution bags, double totalnoInst, double numerator) {
		...
		double result = DecimalCalculate.div(numerator, denumerator) * bags.numBags();
//		System.out.println("GainRatio(S,A) ====> " + result);
		return result;
	}
	...
	private final double splitEnt(Distribution bags, double totalnoInst) {
		...
		if (Utils.gr(bags.total(), 0)) {
			for (i = 0; i < bags.numBags(); i++) {
				// TODO 陈英
				returnValue += (DecimalCalculate.div(bags.perBag(i), totalnoInst) + 0.1) * (bags.total() - bags.perBag(i));
//				returnValue += (DecimalCalculate.div(bags.perBag(i), totalnoInst) + 0) * (bags.total() - bags.perBag(i));
			}
			
//			returnValue = DecimalCalculate.div(returnValue, bags.total());
		}
//		System.out.println("SplitInfo(S,A) ==> " + returnValue);
		return returnValue;
	}
