...
//		System.out.println("GainRatio(S,A) ====> " + result);
		return numerator / denumerator * bags.numBags();
	}
	...
	private final double splitEnt(Distribution bags, double totalnoInst) {
		...
		if (Utils.gr(bags.total(), 0)) {
			for (i = 0; i < numBags; i++) {
				returnValue += bags.perBag(i) * (totalnoInst - bags.perBag(i));
			}
			
			returnValue /= totalnoInst;
//			returnValue = DecimalCalculate.div(returnValue, totalnoInst);
		}
