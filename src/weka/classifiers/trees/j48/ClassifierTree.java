...
public class ClassifierTree implements Drawable, Serializable,
	...
	public void buildTree(Instances data, boolean keepData) throws Exception {
		...
		System.out.println("STEP3.2.1 ==> 选择" + m_toSelectModel.getClass().getName() + "中的selectModel()模型");
	...
	private void graphTree(StringBuffer text) throws Exception {
		...
		for (int i = 0; i < m_sons.length; i++) {
			text.append("N"
					...
					+ m_sons[i].m_id
					+ " [label=\""
					+ Utils.backQuoteChars(m_localModel.rightSide(i, m_train)
							.trim()) + "\"]\n");
			if (m_sons[i].m_isLeaf) {
				text.append("N"
						+ m_sons[i].m_id
						+ " [label=\""
						+ Utils.backQuoteChars(m_localModel.dumpLabel(i,
								m_train)) + "\" " + "shape=box style=filled ");
				if (m_train != null && m_train.numInstances() > 0) {
					text.append("data =\n" + m_sons[i].m_train + "\n");
					text.append(",\n");
				}
				...
				text.append("N"
						+ m_sons[i].m_id
						+ " [label=\""
						+ Utils.backQuoteChars(m_sons[i].m_localModel
								.leftSide(m_train)) + "\" ");
				if (m_train != null && m_train.numInstances() > 0) {
					text.append("data =\n" + m_sons[i].m_train + "\n");
					text.append(",\n");
